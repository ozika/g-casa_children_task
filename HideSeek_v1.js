...
let expName = 'HideSeek_v1';  // from the Builder filename that created this script
let expInfo = {
    'participant': '',
    'study_id': '',
    'session_id': '',
...
function setEnvRoutineBegin(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('setEnv.started', globalClock.getTime());
    ...
    for (const thisComponent of setEnvComponents)
...
function setEnvRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('setEnv.stopped', globalClock.getTime());
...
function skipRoutineRoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    psychoJS.experiment.addData('skipRoutine.started', globalClock.getTime());
    // keep track of which components have finished
    skipRoutineComponents = [];
    
    for (const thisComponent of skipRoutineComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function skipRoutineRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('skipRoutine.stopped', globalClock.getTime());
...
function beforeExpLoopLoopBegin(beforeExpLoopLoopScheduler, snapshot) {
  return async function() {
    ...
    for (const thisBeforeExpLoop of beforeExpLoop) {
...
function mini_instr_trialsLoopBegin(mini_instr_trialsLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisMini_instr_trial of mini_instr_trials) {
...
function trainLoopLoopBegin(trainLoopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisTrainLoop of trainLoop) {
...
function moveLoop_trainLoopBegin(moveLoop_trainLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisMoveLoop_train of moveLoop_train) {
...
function moveLoop_testLoopBegin(moveLoop_testLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisMoveLoop_test of moveLoop_test) {
...
function mini_instr2_trialsLoopBegin(mini_instr2_trialsLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisMini_instr2_trial of mini_instr2_trials) {
...
function getSkipRoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    psychoJS.experiment.addData('getSkip.started', globalClock.getTime());
    ...
    for (const thisComponent of getSkipComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function getSkipRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('getSkip.stopped', globalClock.getTime());
...
function loadparamsRoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    psychoJS.experiment.addData('loadparams.started', globalClock.getTime());
    ...
    for (const thisComponent of loadparamsComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function loadparamsRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('loadparams.stopped', globalClock.getTime());
...
function instrGridRoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    psychoJS.experiment.addData('instrGrid.started', globalClock.getTime());
    ...
    for (const thisComponent of instrGridComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function instrGridRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('instrGrid.stopped', globalClock.getTime());
...
function PathFB_instrRoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    psychoJS.experiment.addData('PathFB_instr.started', globalClock.getTime());
    ...
    for (const thisComponent of PathFB_instrComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
  ...
  return async function () {
    ...
    if (keyFB_3.status === PsychoJS.Status.STARTED) {
      ...
      if (_keyFB_3_allKeys.length > 0) {
        ...
        keyFB_3.duration = _keyFB_3_allKeys[_keyFB_3_allKeys.length - 1].duration;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function PathFB_instrRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('PathFB_instr.stopped', globalClock.getTime());
    ...
    if (typeof keyFB_3.keys !== 'undefined') {  // we had a response
        ...
        psychoJS.experiment.addData('keyFB_3.duration', keyFB_3.duration);
        routineTimer.reset();
        }
...
function setconfigRoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    psychoJS.experiment.addData('setconfig.started', globalClock.getTime());
    ...
    for (const thisComponent of setconfigComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function setconfigRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('setconfig.stopped', globalClock.getTime());
...
function trainGridRoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    psychoJS.experiment.addData('trainGrid.started', globalClock.getTime());
    ...
    for (const thisComponent of trainGridComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function trainGridRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('trainGrid.stopped', globalClock.getTime());
...
function quickfixationRoutineBegin(snapshot) {
  return async function () {
    ...
    // update component parameters for each repeat
    psychoJS.experiment.addData('quickfixation.started', globalClock.getTime());
    // keep track of which components have finished
    quickfixationComponents = [];
    quickfixationComponents.push(text_2);
    
    for (const thisComponent of quickfixationComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function quickfixationRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('quickfixation.stopped', globalClock.getTime());
    // Routines running outside a loop should always advance the datafile row
    if (currentLoop === psychoJS.experiment) {
      psychoJS.experiment.nextEntry(snapshot);
    }
...
function PathFB_trainRoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    psychoJS.experiment.addData('PathFB_train.started', globalClock.getTime());
    ...
    for (const thisComponent of PathFB_trainComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
  ...
  return async function () {
    ...
    if (keyFB_2.status === PsychoJS.Status.STARTED) {
      ...
      if (_keyFB_2_allKeys.length > 0) {
        ...
        keyFB_2.duration = _keyFB_2_allKeys[_keyFB_2_allKeys.length - 1].duration;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function PathFB_trainRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('PathFB_train.stopped', globalClock.getTime());
    ...
    if (typeof keyFB_2.keys !== 'undefined') {  // we had a response
        ...
        psychoJS.experiment.addData('keyFB_2.duration', keyFB_2.duration);
        routineTimer.reset();
        }
...
function checkPerfRoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    psychoJS.experiment.addData('checkPerf.started', globalClock.getTime());
    ...
    for (const thisComponent of checkPerfComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function checkPerfRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('checkPerf.stopped', globalClock.getTime());
...
function testGridRoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    psychoJS.experiment.addData('testGrid.started', globalClock.getTime());
    ...
    for (const thisComponent of testGridComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
  ...
  return async function () {
    ...
    if (key_resp_test.status === PsychoJS.Status.STARTED) {
      ...
      if (_key_resp_test_allKeys.length > 0) {
        ...
        key_resp_test.duration = _key_resp_test_allKeys[_key_resp_test_allKeys.length - 1].duration;
      }
    }
...
function testGridRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('testGrid.stopped', globalClock.getTime());
...
function earlyStopRoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    psychoJS.experiment.addData('earlyStop.started', globalClock.getTime());
    ...
    for (const thisComponent of earlyStopComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function earlyStopRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('earlyStop.stopped', globalClock.getTime());
...
function endExpRoutineBegin(snapshot) {
  return async function () {
    ...
    // update component parameters for each repeat
    psychoJS.experiment.addData('endExp.started', globalClock.getTime());
    ...
    sound_1.setValue(endlate);
    sound_1.secs=1.0;
    sound_1.setVolume(1.0);
    // keep track of which components have finished
    endExpComponents = [];
    endExpComponents.push(bye);
    endExpComponents.push(sound_1);
...
function endExpRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('endExp.stopped', globalClock.getTime());
